...
#define LOG_FILE(outputFile) { \
	OSA::Logger::getInstance().setFile(outputFile); \
	}
 ...
 */
	...
	#define ERROR(msg) { \
		...
		OSA::Logger::getInstance().printOnFile(__FILE__, __LINE__, \
	...
	#define ERROR(msg) { \
		...
		__debug_stream__ << "[ERROR]\t"; \
		__debug_stream__ << msg; \
		OSA::Logger::getInstance().printOnConsole(__FILE__, __LINE__, \
				__debug_stream__.str()); \
		}
#else
	#define ERROR(msg) { \
